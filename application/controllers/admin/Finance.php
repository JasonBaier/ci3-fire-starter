...
class Finance extends Admin_Controller {
    ...
    function add()
    {
		...
		if ($this->form_validation->run() == TRUE)
        {
		 ...
		 $config =  array(
			...
			$saved = $this->finance_model->add_record($this->input->post(), $file, $fileExt, $filepath);
        ...
        $content_data = array(
			...
			'category_list'					=> $this->finance_model->get_categories(),
			'vendor_list'					=> $this->finance_model->get_vendor(),
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->form_validation->run() == TRUE)
        {
			...
			// save the changes
            $saved = $this->finance_model->edit_record($this->input->post(), $file, $fileExt, $filepath);
    ...
    function manage()
    {
        ...
        $content_data = array(
			'category_list'					=> $this->finance_model->get_categories_list(),
			'all_assets'					=> $this->finance_model->get_assets(),
			'vendor_list'					=> $this->finance_model->get_vendor(),
    ...
    function manage_form($id = NULL)
    {
        ...
        if ($this->form_validation->run() == TRUE)
        {
            ...
            $saved = $this->finance_model->edit_catven($this->input->post());
    ...
    function catven()
    {
        ...
        if ($this->form_validation->run() == TRUE)
        {
		 	...
		 	$saved = $this->finance_model->add_catven($this->input->post());
    ...
    {
        if (trim($email) != trim($current) && $this->finance_model->email_exists($email))
        {
    ...
    $data = array();
    $this->load->model('finance_model');
    $query = $this->finance_model->getAllCategories();
    if ($query)
    {
        $data['records'] = $query;
